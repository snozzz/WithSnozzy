...
> 二次元 lofi 插画风格的半身立绘。正面朝向观众，左右对称，中性表情。
>
...
- **双臂垂在身侧**——不用画搭在桌上。房间场景里桌沿从她胸口往下全挡住了，
  手臂根本看不见，于是最难绑的部位直接不用绑。
- **左右对称、中性表情**——绑定的中间态。喜怒哀乐是靠参数变形出来的，不是画出来的。
...
**不需要** `ParamArmL` / `ParamArmR`——手臂被桌沿挡住，不用绑。
