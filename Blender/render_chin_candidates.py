...
#
# 第一轮（p01…p06，已选 p03）**六个都是同一种策略**：四指基本伸直、
# 沿着脸颊往斜上方指，靠指尖去够颧骨。指标全绿，画面上仍读成"举着一只手
# 挡在脸边"——因为手掌在这个机位下比脸还长，指尖够到脸颊时手根还悬在
# 下颌外面，下巴并没有"压"在任何东西上。
#
# 这一轮换一族：**把四指收成松拳，让下颌坐在食指那一节上**。
# 手一握短一半，才塞得进下颌到肩之间那段，而不会一路顶到眼睛
# （`hand_dir` 的 X 分量往里收会盖住眼睛，第一轮就是被这条挡回去的）。
CANDIDATES = [
    # 松拳，腕抬到下颌正下方
    ("q1_fist_low", .17, .045, (.030, -.070, -.105), (.42, -.10, .88), -.18,
     (1.05, 1.15, 1.25, 1.35), -.30, (.10, .03, -.03, -.08)),
    # 拳再往里一点、掌面更朝脸
    ("q2_fist_in", .18, .045, (.024, -.068, -.100), (.36, -.12, .90), -.25,
     (1.10, 1.20, 1.30, 1.40), -.35, (.08, .02, -.03, -.07)),
    # 下颌压在指节上：腕最高，掌面转向脸
    ("q3_knuckle", .19, .050, (.028, -.072, -.096), (.30, -.14, .92), -.30,
     (0.95, 1.08, 1.20, 1.32), -.40, (.10, .03, -.03, -.08)),
    # 半握：食指还留一点伸展贴着颊线，其余收拢
    ("q4_halffist", .17, .045, (.030, -.070, -.108), (.40, -.10, .88), -.22,
     (0.75, 0.95, 1.15, 1.30), -.32, (.14, .04, -.04, -.12)),
    # 收得最紧、位置最高，看会不会顶到眼睛
    ("q5_deep", .20, .050, (.026, -.074, -.092), (.34, -.16, .90), -.28,
     (1.15, 1.25, 1.32, 1.42), -.42, (.08, .02, -.02, -.06)),
    # 对照：p03 原样，用来确认这一轮真的更好而不是换个样子
    ("q6_p03_ref", .17, .050, (.034, -.070, -.115), (.44, -.10, .86), -.18,
