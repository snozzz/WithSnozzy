...
真实桌高 0.73 m 于是桌面在 z=0.725。按这个高度把画上去的桌沿两条行
投回 3D，就得到桌面在进深上的前后位置。桌高不动（那是真实尺寸），
动的只有前后沿。

**换了成品图这两个数就要重算**，办法见 `Blender/fit_scene.py`（几秒钟出数，
不渲染）：它拿量出来的桌沿行做二分，直接解出 y。
这一版用户交付的房间图量到后沿第 608 行、前沿第 735 行，解出
后沿 y=-0.168、前沿 y=-0.794，进深 0.626 m——普通书桌的进深，反推自洽。
（上一版是 602/730 行 → -0.146/-0.761，进深 0.615 m。）
...
#
# 现在（用户交付的房间图这一版）：画上去那块大屏的右缘在画布 x=660，
# 余量比上一版宽裕得多，因为大屏离她远了。
#
# 2026-08-16 从 0.075 挪到 0.055（往她右边 2 厘米）。这不是为了买腕角，
# 是为了**让两条小臂的画面投影对称**：肘往外挪之后（`pose.ELBOW_POLE`）
# 键盘留在原处是左 32 右 39 像素，右臂明显更长；挪 2 厘米之后是 35/34。
# 键盘左缘因此从画布 701 挪到 689，离显示器右缘 660 还有 29 像素——
# **这条硬边界没有余量可挥霍了**（第 40 条），再往那边挪键盘就穿进屏幕里。
# `measure_hands.py` 每次都会报这个数。
CENTRE = Vector((0.055, -0.45, DESK_Z))
