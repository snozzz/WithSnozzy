...
#
# **别拿肘去补偿"右手要横过身体去够"**（第 43 条）。让右肘反着拐确实能把
# 右腕压下来，但肘会缩到肩膀**内侧** 6 厘米、鼓在胸口，人不是那样打字的。
# 两只肘一律摆在身侧（所以这里是一个左右镜像的值，不是字典），
# 右手那笔账由 `HAND_DIR["R"]` 去还。另外小臂会正对镜头——
# 相机在她左前方，小臂一指向她的正前方就整个缩成一个短墩子
# （实测投影只剩 22 像素，满长度是 123），画面上就是两截圆滚滚的袖子
# 后面冒出两只手，没有胳膊。分开摆之后投影回到 33 / 41 像素。
#
# 第三个分量（肘的高低）**不要拿来调画面**（第 42 条）。抬肘确实能把小臂
# 在画面上拉长（+0.2 时投影 95/105），但那是靠**把大臂张开到 60 多度、
# 肘抬到和肩齐平**换来的——人做不出那个姿势，画面上就是肩膀鼓出一大坨。
# 肘必须压在身侧：-1.4 时大臂偏离垂直 34°/22°、肘在肩下方 19/21 厘米，
# 才是坐着打字的样子。小臂投影只有三四十像素是**这个机位的正常结果**，
# 真人正面拍打字也是这样，不是毛病。
