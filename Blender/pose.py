...
# 后面冒出两只手，没有胳膊。分开摆之后投影回到 95 / 105 像素。
#
# 第三个分量（肘的高低）单独说：**它决定小臂在画面上往哪个方向斜**。
# 肘压到和手腕同高（-1.4）腕角最小，但小臂正好躺平指向镜头；
# 抬到 +0.2，小臂在画面上变成从右上往左下的一条斜线（左 124°、右 147°，
# 0° 是画面向右、90° 向下），投影也从 55/63 涨到 95/105。
# 代价是腕角从 12.5/20.7 涨到 20.4/26.2——还在真人打字的范围里，
# 而"没有胳膊"是一眼就看得出来的，这笔换划算。
ELBOW_POLE = {"L": (0.6, 0.6, 0.2), "R": (-0.4, 0.6, 0.2)}
