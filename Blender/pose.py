...
# 低到十几像素才是毛病。别去最大化它。
# 用户报"手臂看着有些扭曲"。量下来毛病是**小臂投影只有 24/23 像素**——
# 第 41 条那条底线是 30，低于它画面上就是"两截袖子后面直接冒出两只手，
# 中间那段胳膊不见了"。原因是这一组把肘几乎直接压在肩下方（Z=-4 主导），
# 小臂于是朝着镜头伸，投影缩成一个短墩子。
#
# 把肘往外挪（X 抬到 1.1/1.3）、别压那么死（Z 收到 -2.8），小臂就有了横向
# 分量，投影 35/34 像素。配合键盘往她右边挪 2 厘米（见 `keyboard.CENTRE`），
# 左右两边还基本对称——只挪肘不挪键盘的话是 32/39，右手明显比左手长。
#
# **别再往外加**：扫过 L1.5/R1.7，投影能到 44/50，但大臂偏离垂直冲到 33–35°，
# 超出真人 15–30° 的范围，肩膀就鼓出来了（第 42 条那个坑）。
# 现在是 28.0°/23.2°，腕角 14.2°/15.5°，都在真人区间里。
ELBOW_POLE = {"L": (1.10, 0.30, -2.8), "R": (1.30, 0.30, -2.8)}
