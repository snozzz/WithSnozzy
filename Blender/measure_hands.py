...
# 画上去的显示器，右缘在画布上的 x。
#
# 这条线是**硬边界**：显示器立在桌面上、比键盘更靠近镜头，本该挡住键盘；
# 可键盘和手是画在桌面层**之上**的，压过去就是键盘穿进显示器里
# （第 40 条）。所以键盘和手的左缘必须全部待在这条线右边。
#
# 重新量的办法（换了重绘图就要重量）：对 `Assets/desk.png` 逐行求横向梯度，
# 在显示器所在的那段 x 里找**最靠右的强边缘**（>40 灰阶）。
# **别用"梯度最大的那一列"**——那报的是"谁最强"，附近的东西一改冠军就换人，
# 显示器根本没动数字也会跳（第 48 条）。
#
# 现在这一版：回折臂折回后墙之后显示器挪到了台面左段，x∈[150,600]、
# y=560/580/600 三行量出来都是 495。比上一版的 580 往左让了 85 像素，
# 键盘的余量只增不减，所以键盘和手不用重渲。
MONITOR_EDGE_X = 495
