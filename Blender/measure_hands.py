...
# 显示器根本没动数字也会跳（第 48 条）。
#
# 现在这一版：照参考效果图重排——L 形工位，左翼前沿退一截做圆角、翼下三抽屉柜，
# 翼上两块屏幕朝镜头的屏，主台面左段一块背对镜头的大屏（第 51 条）。
# rows 360–700、x∈[50,700] 量出来是 573，留点余量记 578。
#
# **扫描窗口要开够宽。** 这一版重绘时桌上冒出过一块多余的立板，右缘在 645，
# 我一开始只扫到 x=600，量出来 589 一路绿灯——那块板其实已经压到键盘上了
# （键盘左缘 594）。窗口开到 700 才看见。
MONITOR_EDGE_X = 578
