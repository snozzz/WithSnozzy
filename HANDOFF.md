...
**只裁"画面某一行以下"会穿模**：袖子挂在肘上、垂得比桌沿低，但肘在桌沿
后面。渲这一层时要在 3D 里摆一块正对镜头的 Holdout 挡板按**深度**切掉
桌沿后面那一段（`snozzy_lib.depth_clip`，切在 2.44）。
高度和骨骼都分不开袖子和手，只有深度可以——见第 29 条。
...
**29. 手画到桌面上会穿模，只能按深度切。** 她那两只蓬蓬袖挂在肘上、
垂到画面 y≈660，而肘在桌沿**后面**（深度 2.55，手腕才 2.39）——
袖子跟着画上去，看着就是小臂插进了桌板。
...
- **不是"身上哪一段"**——袖子和手腕同属小臂那根骨头，按骨骼挑分不开
  （把大臂剔掉重渲，缝线以下的像素一个没少，5321 → 5321）
- **也不能用水平的"桌面"来切**——袖子的 z 是 0.655–0.79、手是 0.698–0.756，
  两者在**高度上重叠**：摆低了什么都挡不住（只去掉 370 个像素），
  摆高了连手指一起削掉（只剩指尖露在键上）

真正的区别在**深度**：袖子在肘上、手伸在前面。所以挡板正对镜头按深度切
（`snozzy_lib.depth_clip`）。

**但光切干净还不够，姿势本身也得对。** 第一版把手腕放在 `KEYS_DIST=2.40`，
手腕在画面上落在 y≈602、**深度却是 2.39——在桌沿后面**。于是整段小臂都在
桌沿背后，袖子悬在桌沿上方的空当里被桌板一刀切平，桌面上只剩两只没有来路
的手。切得再干净也还是穿模，因为**手臂根本没搭到桌上**。

关键是**画面位置和深度要分开看**：手腕在画面上略高于桌沿没关系（那截由
角色图自己画，桌板在那儿本来就是透明的），但**深度必须比桌沿近**，
手才落得到桌面上、和小臂接得起来。

现在的值：`KEYS_DIST=2.30`、`KEYS` 的 v 给 0.575。手腕在画面 y≈593
（桌沿上方一点）、深度 2.29（桌沿前面），肘在 y≈542，指尖落到 y≈636
（键盘中段）。切深度 2.32——2.38 起袖子又开始留在桌上。

两个判据：**指尖要落在键盘的行范围里**（605–665），
**手腕深度要小于切深度**。只看画面位置会漏掉深度那一半。
...
**31. Blender 5 的合成器 API 全变了。** `scene.node_tree` 没了
（改叫 `scene.compositing_node_group`）、`CompositorNodeMapRange` 和
`CompositorNodeMath` 不存在了（数学类统一走 `ShaderNode*`，在合成树里能用）、
`OutputFile` 没有 `base_path` 和 `file_slots` 了。
本来想用深度通道来切手，被这一串挡住，最后改成纯几何的挡板——
反而更稳，跨版本不会再坏。
