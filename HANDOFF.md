...
手落不落得进键盘由三个数决定，都在梯度上试出来的：

- `keyboard.home_row(back=0.058)`——放的是**手腕**，要靠她那一侧。
  手掌和手指是从手腕往前伸一截的，手腕摆在键盘正中，指尖会挂到外沿之外
- `pose.KEY_CURL = 1.0`——打字的手是**拱起来**的。手指伸直会又长又平，
  指尖直接戳出键盘
- 手的朝向必须**转进键盘的坐标系**（`type_hands` 里的 `basis`）。
  键盘是斜放的，照世界方向摆手，手就相对键盘拧了十几度，
  手指斜跨键排、指尖挂到边上去（第 33 条）
...
**34. 手"在不在道具上"是可以量的。** 渲两张：只有键盘的、只有手的，
数手的像素里落在键盘轮廓外的比例。比盯着截图判断可靠得多——
这一条我一开始是靠眼睛看的，来回调了好几轮。
...
- **手在不在道具上**：渲两张（只有键盘的、只有手的），数手的像素里落在
  键盘轮廓外的比例（第 34 条）。这条判据我一开始没做，靠眼睛调了好几轮
