...
重绘对构图的保持度是**实测过的**：窗洞误差 3–9 像素，桌沿/桌腿全部对上。
所以「我定构图、它上色」这条路是可靠的。

**要动构图本身**（挪家具、改桌子形状）而不只是加细节，见第 50 条：
...
当时是 x≈580，回折臂折回后墙、显示器挪到台面左段之后是 x≈495
（第 50 条），换图必须重量。
...
用户要求把 L 形桌那条朝镜头伸的回折臂折到后墙上去。这不是涂改能完的事：
画面左下角整片要变回地板，桌子要往左续到墙角，三块显示器要搬家。

纯文字描述这种改动会跑偏（第二节开头那两轮就是这么废的）。正解是先
**在成品图上粗涂出新几何**再让 codex 画成成品——等于把"灰模 → 重绘"
这条老路搬到局部来用，只是底子从全灰换成了成品图。
...
**取样列要挑干净的**，挑错了就是把椅子腿、杯子、坐垫一起克隆过来，
而且是**重复好几遍**地过来（我先后把落地灯底座和坐垫各铺了三四个）。
挑法：对目标行段逐列算平均横向梯度，最小的那一带就是没有物件轮廓的。
`Scripts/blocking.py` 里那几个 `*_SRC` 常量就是这么定的。

实测结果：主桌中位漂 1px、窗洞 1px、桌上物件位移全是 (0,0)（做互相关量的，
相关 0.85–0.97），左半边整片重画。

**顺带两条：**

- **遮罩要跟着重绘走，而且宁可多包一点。** 重绘把桌子左端面的透视画得比
  灰模陡（那条边从 (170,600) 斜到 (12,704)，灰模只斜到 150）。遮罩短了，
  那一小块桌子就只进房间层、不进桌面层。判法是把 `desk.png` 叠在亮绿底上
  看边界——**多包的部分是墙和地板，那儿没有她，画在桌面层里看不出来；
  少包了却是真漏**。
- **`MONITOR_EDGE_X` 这类"从画面量出来的常数"，换图必须重量**（第 48 条的
  量法）。这次 580 → 495，往左让了 85 像素，键盘余量只增不减，
  所以键盘和手不用重渲——但方向反过来的话就得重渲了，别默认没事。
