...
手腕正好落在桌沿上（画面 y≈602）——于是整段小臂都在桌沿**后面**，
袖子悬在桌沿上方的空当里被桌板一刀切平，桌面上只剩两只没有来路的手。
切得再干净也还是穿模，因为**手臂根本没搭到桌上**。

正解是把手腕推到桌沿**前面**：`KEYS_DIST=2.26`、`KEYS` 的 v 给 0.600，
手腕落在画面 y≈616（桌沿下方）、深度 2.25，肘还在 y≈555（桌沿上方）。
小臂这才真的跨过桌沿搭到桌面上，接得起来。切深度随之改到 2.32。

判据：**肘在桌沿以上、腕在桌沿以下**。两个都在上面就是这次的穿模。
