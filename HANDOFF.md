...
```bash
# 三条长动作（伸懒腰 / 喝咖啡 / 玩手机）：一个脚本管三条，清单在 Scripts/action_defs.py
# 改了 pose.py 里对应的常量，身体和面部贴片**必须一起重出**
blender --background --factory-startup --python Blender/render_action.py -- Snozzy.vrm 输出目录 动作名 2
python3 Scripts/action_frames.py 输出目录 动作名 --out Assets
blender --background --factory-startup --python Blender/render_face.py -- Snozzy.vrm 面部目录 2 动作名
python3 Scripts/face_patches.py 面部目录 --out Assets --prefix face动作名2x --action
# 带道具那两条先量几何，别拿渲图试参数（几秒出数）
blender --background --factory-startup --python Blender/measure_props.py -- Snozzy.vrm both
...
**71.5 「手臂扭曲」的量是小臂投影，不是腕角。**
用户报"手臂看着有些扭曲，不太正常"。腕角 19.1°/17.0°、大臂 23°/19°
全在真人区间里，看指标像是没毛病——**真正出线的是小臂投影 24/23 像素**，
而第 41 条那条底线是 30。低于它画面上就是"两截圆滚滚的袖子后面直接冒出
两只手，中间那段胳膊不见了"，读起来就是别扭。

根子在 `ELBOW_POLE` 的 Z 给到 −4.0，肘几乎直接压在肩正下方，小臂只好
朝着镜头伸。把肘往外挪（X 0.45/0.60 → 1.10/1.30）、别压那么死
（Z → −2.8），投影 32/39。再把键盘往她右边挪 2 厘米
（`keyboard.CENTRE.x` 0.075 → 0.055）**是为了左右对称**，不是为了买腕角：
不挪的话右臂 39、左臂 32，明显不一样长；挪完 35/34。
顺带把第 43 条那个"右肘缩在肩内侧 0.6cm"也修了，现在两肘都在肩外侧。

**别再往外加。** 扫到 L1.5/R1.7 投影能上 44/50，代价是大臂偏离垂直
33–35°，超出真人 15–30°，肩膀就鼓出来了——第 42 条那个坑。
现在 28.0°/23.2°，腕角 14.2°/15.5°，指尖 ±8mm 内，键盘左缘 689、
显示器右缘 660，只剩 29 像素余量（第 40 条那条硬边界没得挥霍了）。

方法上：这一轮是先写一个只报数、不渲染的扫描探针（十几个组合几分钟跑完），
挑出唯一全绿的那组，再用 `measure_hands.py` 复核、渲一张图肉眼确认。
**别拿渲图去试参数**——一轮三四分钟，扫一格几秒钟。

**71.6 `leg_frames.py` 会把托腮/伸懒腰的中间帧当成第六支换腿过渡。**
HANDOFF 让 `render_closeup.py` 和姿势图渲进同一个目录，而两边的中间帧
**都叫 `trans_*_*.png`**。于是 `leg_frames.py` 的通配把 `trans_chin_00…07`
和它的耳机变体一起收了进去，后果三条：

- `seam_residual` 把托腮抬起来的那条胳膊算成"上半身不共用"，报 **0.899%**
  （阈值 0.5%），而只算腿的话是 0.030%。差点据此去挪缝线——**挪缝线是错的**，
  五套腿姿两两相比在缝线以上是**逐像素 0**，共用上半身好得很。
- `alpha_bbox` 会被那条袖子撑大腿的矩形（这次没撑到，纯属运气）。
- 切出一堆 `snozzy_move_chin_*.png`，16 张没人要的图进了库。
...
**73. 画上去的桌上物件，她一件都够不着。**
用户要"喝咖啡"和"拿手机回消息"，桌上正好画着一个马克杯和一台立在支架上的
手机——很自然会想直接拿它们当道具。**实测都够不着**：臂长 0.430 米，
杯子距左肩 0.561、手机 0.588（`/tmp/probe_desk.py` 那一轮）。够不着的目标
会被 IK 夹到伸直、落点悄悄偏掉（第 27 条），画面上是"手伸过去了但杯子
没在手里"。

可达范围是**一个球**：肩在 (0.077, −0.096, 0.981)、半径 0.43，抓握点取桌面
上方 5 厘米，交出来的桌面圆半径只有 0.30 左右——差不多就是键盘周围一圈。
所以道具只能建在那一圈里（`Blender/props.py`），画上去那个杯子用
`dewatermark.py` 横向克隆抹掉（那块桌面干净、纹理是水平的，一次就干净）。
**手机没抹**：它连着支架、线和旁边的平板，抹了会带出一片糊，
留着当场景里的第二台设备。

**74. `join` 出来的道具带着建模时的缩放，拿它求刚性变换会炸。**
第 34 条说过量的时候别拿 `matrix_world` 反变换，这次是**摆**的时候撞上：
手机是两个立方体 `join` 的，活下来那个带着 `scale=(0.071, 0.146, 0.009)`。
"把杯子/手机从桌上刚性搬到嘴边"这件事算的是 `目标 × 静置⁻¹`，静置矩阵里
混着 1/0.009 这种系数，逆一下整个变换就飞了——判据报的是手机跑到画布外
几十万像素处。修法一行：`transform_apply(scale=True)` 把缩放烘进网格。

**75. 姿势函数不能拿"道具现在在哪"当静置位置。**
第一版 `pose.coffee` 开头是 `mug_rest = mug.matrix_world.copy()`。渲染脚本
每帧重载场景，看不出问题；判据在同一个场景里连摆八档，第二档起读到的
"静置位置"是上一档留在她手里的位置，算出来的刚性变换≈单位阵，
**手就停在抓握姿势上不动了**。

暴露它的是判据里那八行数字**一模一样**——不是某个数不对，是"根本没变化"。
静置位置改成常量（`props.REST`）并且每次开头先 `reset()`，姿势函数就幂等了。

**76. 举上去冻两秒再放下来，就是做操。**
用户报伸懒腰"像某种做操动作，很僵硬"。渲出来一看，三件事凑在一起：
两条胳膊**笔直**举成对称的 V、手掌摊开五指张着、头是平的——每一样单看都
说得过去，合起来就是广播体操。真人伸懒腰是**肘弯着把手送到脑后、
手松松攥起来、下巴抬起来**，而且两条胳膊从来不是镜像的。

更要命的是**中间那两秒是一张静止图**。动作的时间轴原来是"举上去 →
冻住 → 放下来"，三拍，正是体操的节奏。所以现在每条长动作都多一列
`hold` 帧（`Scripts/action_defs.py` 的 `holds`），运行时循环播：
伸懒腰是脖子左右晃 + 胸口跟着呼吸起伏，喝咖啡是小口啜，玩手机是拇指点屏幕。
判据也跟着分成两段——中间帧是大动作、hold 是小幅摆动，混在一起算相邻
XOR 峰值比必然爆掉，而那不是毛病（第 45 条）。

**77. 杯子挡住嘴，不是难看，是这套动作直接做不出来。**
`face_patches.py` 要求 13 个变体**块块有变化**（少一块整套贴片就不完整）。
杯口压在嘴上那一档，`mouth_open` 和中性底图逐像素相同，它直接报
"无变化，不能作为完整贴片"——于是这条动作的贴片出不来，运行时整套不启用。

所以杯口停在下唇**下方三厘米**（`pose.COFFEE_RIM_OFFSET`），画面上读到的
仍然是"举到嘴边"。这个距离是扫出来的：0.048 米时 hold 的某一相位刚好压上去
（判据报 53 个顶点），0.062 全绿。**每一相位都要单独查**，只查终态会漏掉
幅度最大的那一档。

**78. 抓握姿势决定了能不能喝，不是"抓住了就行"。**
第一版让她从**上方**扣住杯子（手腕在杯口上方五厘米）：伸手那一段很自然，
可是杯子挂在手底下，要把杯口送到嘴边，肘就必须抬到肩膀上面——实测大臂
偏离垂直 123°、肘高过肩 12 厘米，画面上是一只鸡翅膀。

改成从**近侧握杯身**，杯子就在手的前上方，举到嘴边时小臂自然立起来、
肘垂在身侧（55°、肘低于肩 13 厘米）。**摆道具上的手，要先问"拿着它做完
整个动作"，不是只看抓的那一帧**。

**79. 手那一层的裁切上沿是给手臂定的规矩，杯子比桌沿高。**
`hand_frames.py` 一直把裁切矩形的上沿钉在"桌面层第一次有像素"那一行
（第 60 条那条安全线：这一层只画桌沿以下的东西）。杯子有 9.5 厘米高，
顶端在那条线**以上** 35 个像素——照旧规矩裁，桌上那个杯子是被削掉一截的。

往上撑到杯口是安全的，理由要说清楚：那几行桌面层根本没有像素，
不存在"盖到桌子上面"这回事；而那几行里的小臂像素和角色层完全一样
（同一台相机同一个姿势），画两遍也看不出来。`props_only.png` 是渲染脚本
专门为这件事多渲的一张——**从像素量，不在 Blender 里算画布坐标**。

**80. 深夜一直闭着眼不是打瞌睡，是死机。**
困倦原来只做了一件事：`blink = max(blink, sleepy)`。于是凌晨她就是闭着眼
坐在那儿一动不动——读起来是"睡着了"，甚至像画面卡住了。

真正读得出打盹的是**那个循环**：头一点一点往下沉、眼皮跟着合上，
沉到底猛地一惊抬起来、眼睛短暂睁大、视线弹回屏幕，过几秒又开始沉
（`DrowsyRig.nod`，11 秒一轮，周期本身还在漂，免得像节拍器）。
渲染版没有骨骼可驱动，所以"点头"是整幅图往下挪十来像素 + 前倾 1.6°，
和呼吸摇摆同一个路子。**身体和脸读同一份公式**——各算一遍的话眼睛和头
会各点各的，比不动更怪。

**81. 同一个 app 里存在两个长得完全不一样的 Snozzy。**
完整窗口早就换成 Blender 渲染的她了，迷你播放器和桌宠一直还画着矢量简笔版
（`CharacterView`）。这是第 70 条的反面：那次是"没人画的那份悄悄旧了"，
这次是有人画，但画的是另一个人。

修法不是再渲一套半身素材，而是**把整幅画布放大、裁出胸像那一块**
（`RenderedBust`）：角色层本来就是 2× 密度的，裁出来仍然清楚。
...
- **三条长动作**（`Features/ActionRig.swift` 一个播放器管三条，
  素材清单 `Scripts/action_defs.py`）：

  - **伸懒腰**：每隔 5–10 分钟自发一次（每次重新抽间隔，固定周期几轮就能
    预判），专注段自然结束时也来一个。肘弯着把手送到脑后、手松松攥起来、
    胸口打开、下巴抬起来，两条胳膊**不对称**
  - **喝咖啡**：进入专注段时端起杯子喝一口（`FocusTimer.onPhaseBegan`，
    手动 skip 不触发），另外每 15–30 分钟自发一次
  - **玩手机**：拿起手机、拇指敲屏幕。**不自发**，等外面戳一下——
    `touch ~/Library/Application\ Support/WithSnozzy/phone.nudge`
    （`PhoneNudge`，1.2 秒轮询）。用户回微信/TG 时把这行接上去，
    她就同步拿起手机

  三条的时间轴都是 常态 → 八张中间帧 → 终态 → **一列循环的停留帧** →
...
- **动作面板**（控制条上那个招手图标）：把所有能主动演的东西列出来，
  点一下就演——托腮、三条长动作、五个活动档位（按住不放就一直是那一档）、
...
**现在值不值得切过去：不值得，但这条线要留着。** 2026-08-16 实测过一轮，
`--3droom-snapshot` 十五道 gate 全绿、加载 898 毫秒、活动帧 56.7 fps
（帧时中位 18 毫秒），**技术上完全跑得通**。问题不在性能，在美术：
房间现在还是**灰模块**——书是彩色方块、墙是一块平板、没有贴图没有布光，
而 2.5D 那边是一张完成度很高的插画（灯串、霓虹、绿植、窗外城市）。
把 3D 拉到同一水平，等于把整间房重新做成带贴图和布光的 3D 场景，
正是第五节原来那句"要重建插画房间、材质"的成本。

所以分界线很清楚：**固定机位下，2.5D 赢在画面，而画面正是这个 app 的全部。**
3D 真正换来的是两样 2.5D 给不了的东西——**自由镜头**，
以及**长动作**（`stand_stretch_once` 这类整段身体动作在 2.5D 里
每一套都要重渲一整份上半身素材）。哪天要这两样里的任何一样，再评估。
