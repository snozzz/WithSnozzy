...
**肘的高低单独说：它决定小臂在画面上往哪边斜。** 肘压到和手腕同高
腕角最小，但小臂正好躺平指向镜头；抬起来（`ELBOW_POLE` 第三个分量
从 -0.6 到 +0.2）小臂就变成从右上往左下的一条斜线，投影 55/63 → 95/105。
代价是腕角 12.5/20.7 → 20.4/26.2，还在真人范围里。
**"没有胳膊"是一眼看得出来的，腕角多十度不是**，所以这笔换划算。
...
- **手在不在道具上**：`Blender/measure_hands.py`，几秒钟出数，不渲染。
  ...
  **小臂在画面上投影多长**（<30px 就是正对镜头、没有胳膊，第 41 条），
