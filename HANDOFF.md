...
**51. 朝里退的东西必然被透视压扁，这不是画小了。** 同样一条回折臂，
朝镜头来是越走越大（原来一路铺到画幅底边，占掉整个左下角），朝里退是
越走越小（现在是贴着左墙的一条窄台，比主台面高一点——离地平线近）。
臂上的显示器也跟着变小、上移。事先说清楚这一点，免得成品出来被当成画错了。
...
**屏幕斜着朝她**（约 45°，`SCREENS` 的 `lean` 取负＝顶往左让）比正对镜头
或者全背对都好：既读得出是给她看的，镜头也能看见一部分屏幕内容——
"屏幕跟播放状态联动发光"那个待办要靠这个才做得了。
