...
真正的区别在**深度**：袖子在 2.5，手在 2.39。所以挡板正对镜头按深度切
（`snozzy_lib.depth_clip`，切在 2.44）。拉梯度看出来的：
2.48 袖子开始露出来，2.40 以下开始啃手腕。

**我在这条上绕了一圈**：先做对了（正对镜头的深度挡板），又"推理"成水平桌面，
反而更糟。教训是——这种几何问题**不要靠推理，渲一张把挡板涂红了看它在哪**。
