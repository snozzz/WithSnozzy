...
用户不想接 OpenAI 的 API，但有 **Plus 订阅**，问能不能拿订阅号说话。
**能**，而且不用 API key、不按量计费：订阅本来就带一个跑在本机的命令行，
直接驱动它就行。

- `claude -p`（Claude Code CLI，走 Claude 订阅）——**默认走这个**
- `codex exec`（Codex CLI，走 GPT Plus；重绘管线一直在用它）
