...
后面。所以渲这一层时要在 3D 里摆一块水平的 Holdout 挡板替 2D 的桌子挡住
（`snozzy_lib.desk_occluder`，高度按指尖取），见第 29 条。
...
**29. 手画到桌面上会穿模，得在 3D 里补一块桌子挡住。** 只按"画面第几行以下
画到桌面上"来切是不够的：她那两只蓬蓬袖挂在肘上、垂到画面 y≈660，
而肘在桌沿**后面**（深度 2.55，手腕才 2.39）——袖子跟着画上去，
看着就是小臂插进了桌板。

这条分界**既不是画面上哪一行**（袖子和手指落在同样的行上），
**也不是身上哪一段**（袖子和手腕同属小臂那根骨头，按骨骼挑分不开），
只能按几何切。做法是在 3D 里摆一块 Holdout 材质的挡板
（`snozzy_lib.desk_occluder`），位置从 2D 桌面上沿反推。

挡板必须**水平**，不能正对镜头：桌面是个平面，画面越往下深度越近，
"该切在多深"随行变化。用一块固定深度的挡板切，上边切多了下边切少了。
高度要按**指尖**取，不是手腕——手腕比指尖高五公分，照手腕摆会把手指整个削掉
（试过，只剩指尖还露在键上）。
